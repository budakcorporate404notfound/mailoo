...
<nav aria-label="breadcrumb">
    <ul class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Menimbang</li>
    </ul>
</nav>
@endsection
        ...
        <div class="iq-card">
            <div class="iq-card-header d-flex justify-content-between">
                <div class="iq-header-title">
                    <h4 class="card-title">Data Referensi Surat Tugas (Menimbang)</h4>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="productForm" name="productForm" class="form-horizontal">
                        ...
                        <div class="form-row">
                            <div class="col">
                                <label for="urutan">urutan <span style="color:red;font-weight:bold">*</span></label>
                                <input id="urutan" name="urutan" required="" placeholder="urutan referensi ..."
                        ...
                        <div class="form-row">
                            <div class="col">
                                <label for="text_menimbang">text menimbang <span
                                        style="color:red;font-weight:bold">*</span></label>
                                <textarea id="text_menimbang" name="text_menimbang" required="" class="form-control"
                                    placeholder="text menimbang ..." autocomplete="on"></textarea>

                                @error('text_menimbang')
                                <span class="text-danger"> {{$message}} </span>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="col">
                            <div class="iq-card-body" id="locked">
                                <div
                                    ...
                                    <label class="custom-control-label" for="customRadio-3"><i class="las la-lock"></i>
                                        text terkunci </label>
                                </div>
...
<script type="text/javascript">
    $(function() {
        ...
        $('#createNewProduct').click(function() {
            ...
            $('#modelHeading').html("Tambah referensi menimbang");
            $('#ajaxModel').modal('show');
        });
        $('body').on('click', '.editProduct', function() {
            var product_id = $(this).data('id');
            $.get("{{ route('menimbang.index') }}" + '/' + product_id + '/edit', function(data) {
                $('#modelHeading').html("edit referensi menimbang");
                $('#saveBtn').val("edit-user");
                $('#ajaxModel').modal('show');
                $('#product_id').val(data.id);
                // $('#id_unitbagian').val(data.id_unitbagian);
                $('#urutan').val(data.urutan);
                $('#text_menimbang').val(data.text_menimbang);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-info rounded-pill"><i class="ri-heart-fill"></i> tambah atau edit refensi menimbang berhasil <i class="ri-heart-fill"></i></button>'
                            );
                            ...
                            $('#saveBtn').html('Save Changes');
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-danger rounded-pill"><i class="las la-skull-crossbones"></i> tambah atau edit referensi menimbang gagal <i class="las la-skull-crossbones"></i></button>'
                            );
                            $("#response").fadeTo(5000, 500).slideUp(500, function() {
                                $("#response").slideUp(500);
                            });
        ...
        $('body').on('click', '.deleteProduct', function() {
            var product_id = $(this).data("id");
            var result = confirm("kamu ingin delete referensi menimbang ini !");
            if (result) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('menimbang.store') }}" + '/' + product_id,
                    success: function(data) {
                        if (data.success) {
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-warning rounded-pill"><i class="las la-exclamation-triangle"></i> data refensi menimbang telah dihapus <i class="las la-exclamation-triangle"></i></button>'
                            );
                            $("#response").fadeTo(5000, 500).slideUp(500, function() {
                                $("#response").slideUp(500);
                            });
                            ...
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-danger rounded-pill"><i class="las la-skull-crossbones"></i> data refensi menimbang gagal dihapus <i class="las la-skull-crossbones"></i></button>'
                            );
                            table.draw();
                        }
                    },
