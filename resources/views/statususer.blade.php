...
<nav aria-label="breadcrumb">
    <ul class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Status User</li>
    </ul>
</nav>
@endsection
                ...
                <div class="modal-body">
                    <form id="productForm" name="productForm" class="form-horizontal">
                        ...
                        <div class="div form-row">
                            <div class="col">
                                <label for="email"> email <span style="color:red">read only</span></label>
                                <input id="email" name="email" required="" readonly placeholder="user email ..."
                                    class="form-control">
                                @error('email')
                                <span class="text-danger"> {{$message}} </span>
                                @enderror
                                ...
                                <div class="iq-card-body" id="locked">
                                    <div
                                        ...
                                        <label class="custom-control-label" for="customRadio-3"><i
                                                class="las la-lock"></i> akun terkunci </label>
                                    </div>
                                    ...
                                    <div
                                        ...
                                        <label class="custom-control-label" for="customRadio-6"><i
                                                class="las la-lock-open"></i> akun terbuka </label>
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(function() {
        ...
        $('body').on('click', '.editProduct', function() {
            ...
            $.get("{{ route('statususer.index') }}" + '/' + product_id + '/edit', function(
                data) {
                $('#modelHeading').html("edit status user");
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            $("#jabatan").val(null).trigger(
                                "change");
                            $("#unit_kerja").val(null).trigger(
                                "change");
        ...
        $('body').on('click', '.deleteProduct', function() {
            var product_id = $(this).data("id");
            var result = confirm("kamu ingin delete user ini !");
            if (result) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('statususer.store') }}" + '/' + product_id,
                    success: function(data) {
                        if (data.success) {
                            $('#response').html(
