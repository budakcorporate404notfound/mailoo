...
<nav aria-label="breadcrumb">
    <ul class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Untuk</li>
    </ul>
</nav>
@endsection
        ...
        <div class="iq-card">
            <div class="iq-card-header d-flex justify-content-between">
                <div class="iq-header-title">
                    <h4 class="card-title">Data Referensi Surat Tugas (Untuk)</h4>
                ...
                <div>
                    ...
                    <a class="btn btn-primary rounded-pill mb-3" href="javascript:void(0)" id="createNewProduct"
                        title="tambah referensi untuk"> + </a>
                </div>
                @break
                @case(6)
                <div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="productForm" name="productForm" class="form-horizontal">
                        ...
                        <div class="form-row">
                            <div class="col">
                                <label for="untuk">text untuk <span style="color:red;font-weight:bold">*</span></label>
                                <textarea id="text_untuk" name="text_untuk" required="" class="form-control"
                                    placeholder="text untuk ..." autocomplete="on"></textarea>
...
<script type="text/javascript">
    $(function() {
        ...
        $('#createNewProduct').click(function() {
            ...
            $('#modelHeading').html("Tambah referensi untuk");
            $('#ajaxModel').modal('show');
        });
        $('body').on('click', '.editProduct', function() {
            var product_id = $(this).data('id');
            $.get("{{ route('untuk.index') }}" + '/' + product_id + '/edit', function(data) {
                $('#modelHeading').html("edit referensi untuk");
                $('#saveBtn').val("edit-user");
                $('#ajaxModel').modal('show');
                $('#product_id').val(data.id);
                // $('#id_unitbagian').val(data.id_unitbagian);
                // $('#urutan').val(data.urutan);
                $('#text_untuk').val(data.text_untuk);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-info rounded-pill"><i class="ri-heart-fill"></i> tambah atau edit refensi untuk berhasil <i class="ri-heart-fill"></i></button>'
                            );
                            ...
                            $('#saveBtn').html('Save Changes');
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-danger rounded-pill"><i class="las la-skull-crossbones"></i> tambah atau edit referensi untuk gagal <i class="las la-skull-crossbones"></i></button>'
                            );
                            $("#response").fadeTo(5000, 500).slideUp(500, function() {
                                $("#response").slideUp(500);
                            });
        ...
        $('body').on('click', '.deleteProduct', function() {
            var product_id = $(this).data("id");
            var result = confirm("kamu ingin delete referensi untuk ini !");
            if (result) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('untuk.store') }}" + '/' + product_id,
                    success: function(data) {
                        if (data.success) {
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-warning rounded-pill"><i class="las la-exclamation-triangle"></i> data refensi untuk telah dihapus <i class="las la-exclamation-triangle"></i></button>'
                            );
                            $("#response").fadeTo(5000, 500).slideUp(500, function() {
                                $("#response").slideUp(500);
                            });
                            ...
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-danger rounded-pill"><i class="las la-skull-crossbones"></i> data refensi untuk gagal dihapus <i class="las la-skull-crossbones"></i></button>'
                            );
                            table.draw();
                        }
                    },
