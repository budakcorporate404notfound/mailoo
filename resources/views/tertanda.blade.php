...
<nav aria-label="breadcrumb">
    <ul class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Tertanda</li>
    </ul>
</nav>
@endsection
        ...
        <div class="iq-card">
            <div class="iq-card-header d-flex justify-content-between">
                <div class="iq-header-title">
                    <h4 class="card-title">Data Referensi Surat Tugas (Tertanda)</h4>
                ...
                <div>
                    ...
                    <a class="btn btn-primary rounded-pill mb-3" href="javascript:void(0)" id="createNewProduct"
                        title="tambah referensi tertanda"> + </a>
                </div>
                    ...
                    <form id="productForm" name="productForm" class="form-horizontal">
                        ...
                        <div class="form-row">
                            <div class="col">
                                ...
                                <input id="nama" name="nama" required="" class="form-control" placeholder="nama ..." autocomplete="on">
                            ...
                            <div class="col">
                                <label for="nip">nip <span style="color:red;font-weight:bold">*</span></label>
                                <input id="nip" name="nip" required="" class="form-control" placeholder="nip ..." autocomplete="on">
...
<script type="text/javascript">
    $(function() {
        ...
        $('#createNewProduct').click(function() {
            ...
            $('#modelHeading').html("Tambah referensi tertanda");
            $('#ajaxModel').modal('show');
        });
        $('body').on('click', '.editProduct', function() {
            var product_id = $(this).data('id');
            $.get("{{ route('tertanda.index') }}" + '/' + product_id + '/edit', function(data) {
                $('#modelHeading').html("edit referensi tertanda");
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-info rounded-pill"><i class="ri-heart-fill"></i> tambah atau edit tertanda untuk berhasil <i class="ri-heart-fill"></i></button>'
                            );
                            ...
                            $('#saveBtn').html('Save Changes');
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-danger rounded-pill"><i class="las la-skull-crossbones"></i> tambah atau edit tertanda untuk gagal <i class="las la-skull-crossbones"></i></button>'
                            );
                            $("#response").fadeTo(5000, 500).slideUp(500, function() {
                                $("#response").slideUp(500);
                            });
        ...
        $('body').on('click', '.deleteProduct', function() {
            var product_id = $(this).data("id");
            var result = confirm("kamu ingin delete referensi tertanda ini !");
            if (result) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('tertanda.store') }}" + '/' + product_id,
                    success: function(data) {
                        if (data.success) {
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-warning rounded-pill"><i class="las la-exclamation-triangle"></i> data refensi tertanda telah dihapus <i class="las la-exclamation-triangle"></i></button>'
                            );
                            $("#response").fadeTo(5000, 500).slideUp(500, function() {
                                $("#response").slideUp(500);
                            });
                            ...
                            $('#response').html(
                                '<button type="button" class="btn mb-3 btn-danger rounded-pill"><i class="las la-skull-crossbones"></i> data refensi tertanda gagal dihapus <i class="las la-skull-crossbones"></i></button>'
                            );
                            table.draw();
                        }
                    },
